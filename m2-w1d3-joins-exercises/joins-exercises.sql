...
--Below is to show my process as I am just not understanding how to set up the queries without yielding errors and getting seriously stuck

SELECT r.rental_id AS rental_count, f.*, inv.*, r.*
FROM film f
JOIN inventory as inv ON inv.film_id = f.film_id
JOIN rental as r ON r.inventory_id = inv.inventory_id 
--works
...
SELECT top 10 f.title AS title, f.*, inv.*, r.*
FROM film f
JOIN inventory as inv ON inv.film_id = f.film_id
JOIN rental as r ON r.inventory_id = inv.inventory_id
--works

SELECT top 10 f.title AS title, count(r.rental_id) AS rental_count, f.*, inv.*, r.*
FROM film f
JOIN inventory as inv ON inv.film_id = f.film_id
JOIN rental as r ON r.inventory_id = inv.inventory_id
--no longer works

SELECT top 10 f.title AS title, r.rental_id AS rental_count, f.*, inv.*, r.*
FROM film f
JOIN inventory as inv ON inv.film_id = f.film_id
JOIN rental as r ON r.inventory_id = inv.inventory_id
GROUP BY f.title
